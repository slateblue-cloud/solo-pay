...
| **4. 컨트랙트 실행** | 블록체인     | `PaymentGateway.payWithSignature()` 실행 → 토큰 에스크로                                                       |
| **5. Webhook 알림**  | SoloPay 서버 | payment.escrowed, payment.finalized, payment.cancelled 또는 payment.failed/expired를 가맹점 Webhook URL로 전송 |
