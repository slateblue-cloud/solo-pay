...
import { cancelPaymentRoute } from '../cancel';
...
describe('POST /payments/:id/cancel', () => {
  ...
  describe('successful responses', () => {
    it('should return 200 with cancel data for valid escrowed payment', async () => {
      ...
      const serverSignature = '0x' + 'd'.repeat(130);
      ...
      mockSigningService.signCancelRequest.mockResolvedValue(serverSignature);
      mockRelayerService.submitDirectTransaction.mockResolvedValue(mockRelayResult);

      const response = await app.inject({
        method: 'POST',
        url: `/payments/${paymentHash}/cancel`,
