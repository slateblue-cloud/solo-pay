...
import { finalizePaymentRoute } from '../../../src/routes/payments/finalize';
...
const createMockPayment = (overrides = {}) => ({
  ...
  escrow_deadline: new Date(Date.now() + 3600_000), // 1 hour from now
  ...overrides,
});
...
describe('POST /payments/:id/finalize', () => {
  ...
  beforeEach(async () => {
    ...
    signingService = {
      signFinalizeRequest: vi.fn().mockResolvedValue(MOCK_SIGNATURE),
    };
  ...
  describe('Success cases', () => {
    it('should return 200 with signature for valid ESCROWED payment', async () => {
      const response = await app.inject({
        method: 'POST',
        url: `${API_V1_BASE_PATH}/payments/${VALID_PAYMENT_ID}/finalize`,
    ...
    it('should create FINALIZE_SUBMITTED event', async () => {
      await app.inject({
        method: 'POST',
        url: `${API_V1_BASE_PATH}/payments/${VALID_PAYMENT_ID}/finalize`,
        headers: { 'x-api-key': TEST_API_KEY },
      ...
      expect(paymentService.createEvent).toHaveBeenCalledWith(1, 'FINALIZE_SUBMITTED');
    });
  });
  ...
  describe('Error cases', () => {
    ...
    it('should return 400 for invalid paymentId format (not bytes32)', async () => {
      const response = await app.inject({
        method: 'POST',
        url: `${API_V1_BASE_PATH}/payments/not-a-bytes32/finalize`,
        headers: { 'x-api-key': TEST_API_KEY },
      });
    ...
    it('should return 400 when escrow deadline has expired', async () => {
      paymentService.findByHash = vi
        .fn()
        .mockResolvedValue(createMockPayment({ escrow_deadline: new Date(Date.now() - 3600_000) }));
    ...
    it('should return 500 when signing service throws', async () => {
      signingService.signFinalizeRequest = vi.fn().mockRejectedValue(new Error('Signing failed'));
